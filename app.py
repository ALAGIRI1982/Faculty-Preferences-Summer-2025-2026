...
def generate_pdf(emp_id, name, designation, b1, b2):
    ...
    content.append(Paragraph("Faculty Course Preference Receipt", styles['Title']))
    content.append(Spacer(1, 20))

    content.append(Paragraph(f"Employee ID: {emp_id}", styles['Normal']))
    content.append(Paragraph(f"Name: {name}", styles['Normal']))
    content.append(Paragraph(f"Designation: {designation}", styles['Normal']))
    content.append(Spacer(1, 20))

    content.append(Paragraph("Basket 1:", styles['Heading2']))
    for c in b1:
        content.append(Paragraph(f"- {c}", styles['Normal']))

    content.append(Spacer(1, 10))

    content.append(Paragraph("Basket 2:", styles['Heading2']))
    for c in b2:
        content.append(Paragraph(f"- {c}", styles['Normal']))
...
if page == "success":
    ...
    <div style="display:flex; justify-content:center; align-items:center; height:80vh;">
        <div style="background:#16a34a; color:white; padding:40px 80px;
...
if page == "summary":
    ...
    pdf_path = generate_pdf(emp_id, name, designation,
                            st.session_state.get("b1", []),
                            st.session_state.get("b2", []))

    with open(pdf_path, "rb") as f:
        st.download_button("📥 Download Receipt PDF", f,
                           file_name="Faculty_Preference_Receipt.pdf")
...
b1_selected = st.multiselect("Basket 1", b1_df["Course"], key="b1", on_change=handle_b1)
b2_selected = st.multiselect("Basket 2", b2_df["Course"], key="b2", on_change=handle_b2)

st.write(f"Basket1: {len(b1_selected)}/7 | Basket2: {len(b2_selected)}/7")
...
if st.button("Submit"):
    ...
    if len(b1_selected) != 7 or len(b2_selected) != 7:
        st.error("Select exactly 7 courses")
        st.stop()
