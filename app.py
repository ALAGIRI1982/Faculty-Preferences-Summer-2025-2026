...
# -----------------------------
st.set_page_config(
    page_title="ERP Faculty Preference Dashboard",
    layout="wide"
)
...
st.markdown("""
    <div style="
        ...
        font-size:24px;
        font-weight:bold;">
        ERP FACULTY PREFERENCE DASHBOARD
    </div>
""", unsafe_allow_html=True)
...
preferences = st.multiselect(
    "Select Course Preferences (Max 5)",
    ["AI", "ML", "DBMS", "CN", "OS", "DS", "SE", "IOT"]
)
...
def generate_pdf(data):
    ...
    title = Paragraph("Faculty Preference Report", styles['Title'])
...
# -----------------------------
# SUBMIT SECTION (INLINE STATUS)
# -----------------------------
colA, colB, colC = st.columns([1.2, 1, 2])

with colA:
    submit = st.button("✅ Submit Preference")
...
# -----------------------------
if submit:
    if emp_id and name and preferences:
        data = [
            ["Employee ID", emp_id],
            ["Name", name],
            ["Department", dept],
        ...
        sheet.append_row([emp_id, name, dept, email, ", ".join(preferences)])
        ...
        status_placeholder.markdown(
            "<div style='color:green;font-weight:bold;'>✔ Submitted Successfully</div>",
            unsafe_allow_html=True
        )
...
# -----------------------------
# DOWNLOAD BUTTON (RIGHT SIDE SAME LINE)
# -----------------------------
with download_btn_placeholder:
    if st.session_state.submitted:
        st.download_button(
            ...
            file_name="faculty_preference_report.pdf",
            mime="application/pdf"
        )
...
st.markdown("""
    ...
    <div style="text-align:center;color:gray;">
        ERP System © 2026 | Faculty Preference Module
    </div>
""", unsafe_allow_html=True)
