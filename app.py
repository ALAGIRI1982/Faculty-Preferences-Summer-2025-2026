...
with col1:
    ...
    b1_selected = st.multiselect("Select exactly 7 courses", b1_courses, max_selections=7)
    st.write(f"Selected: {len(b1_selected)} / 7")

    threshold = b1_df["Usage"].median()
...
with col2:
    ...
    b2_selected = st.multiselect("Select exactly 7 courses", b2_courses, max_selections=7)
    st.write(f"Selected: {len(b2_selected)} / 7")

    threshold = b2_df["Usage"].median()
...
with col1:
    ...
    b1_selected = st.multiselect("Select up to 7 courses", b1_courses)
    st.write(f"Selected: {len(b1_selected)} / 7")

    if len(b1_selected) > 7:
        st.error("❌ You can select only 7 courses")
...
with col2:
    ...
    b2_selected = st.multiselect("Select up to 7 courses", b2_courses)
    st.write(f"Selected: {len(b2_selected)} / 7")
